...
local config = {
	...
	renderer = { -- UI rendering setup
		...
		highlight_opened_files = "all", -- Highlight icons and/or names for opened files. value: `none`, `icon`, `name` or `all`
		root_folder_modifier = ":~", -- :help filename-modifiers
	...
	git = { -- Git integration with icons and colors.
		...
		timeout = 300, -- Kills the git process after some time if it takes too long.
	},
	...
	actions = { -- Configuration for various actions.
		...
		file_popup = { -- Configuration for file_popup behaviour.
			open_win_config = { -- Floating window config for file_popup. See |nvim_open_win| for more details.  [???]
				col = 1,
				row = 1,
				relative = "cursor",
				border = "shadow",
