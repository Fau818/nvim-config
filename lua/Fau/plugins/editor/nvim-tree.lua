...
local function _smart_open(mode)
  ...
  local function fallback(node)
    ...
    else Fau_vim.notify("nvim-tree: Unknown open mode!", vim.log.levels.ERROR)
    end
  end

  -- Get basic information.
  local node = api.tree.get_node_under_cursor()
  if not node then Fau_vim.notify("nvim-tree: Get node info error!", vim.log.levels.ERROR) return end
  ...
  if node.type == "file" then
    ...
    if result == "binary" then
      Fau_vim.notify("Binary file, open it in system. (Use `o` to force edit it in neovim)", vim.log.levels.INFO)
...
return {
  {
    ...
    opts = {
      ...
      git = {
        ...
        disable_for_dirs = {},
        timeout = Fau_vim.settings.timeout.git,
      ...
      actions = {
        ...
        expand_all = { max_folder_discovery = Fau_vim.file.large_folder_count, exclude = {} },
