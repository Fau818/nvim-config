...
local format_config = {
  ...
  end_of_line = "auto",  ---@type "crlf"|"lf"|"cr"|"auto"
  ...
  call_arg_parentheses = "always",  ---@type "keep"|"remove"|"remove_table_only"|"remove_string_only"|"always"
  ...
  table_separator_style = "comma",  ---@type "none"|"comma"|"semicolon"|"only_kv_colon"
  trailing_table_separator = "smart",  ---@type "keep"|"never"|"always"|"smart"
...
local settings = {
  Lua = {
    ...
    diagnostics = {
      ...
      ignoredFiles = "Opened",  ---@type "Enable"|"Opened"|"Disable" Set how files that have been ignored should be diagnosed.
      libraryFiles = "Opened",  ---@type "Enable"|"Opened"|"Disable" Set how files loaded with workspace.library are diagnosed.

      -- workspaceDelay = -1,  -- NOTE: Keep it enabled, otherwise `lazydev.nvim` will not work properly (no diagnostics auto refresh).
      workspaceEvent = "OnSave",  ---@type "OnChange"|"OnSave"|"None" Set when the workspace diagnostics should be analyzed.
    ...
    hint = {              -- Settings for configuring inline hints
      ...
      awaitPropagate = false,  --  When a function calls a function marked @async, it will be automatically marked as @async.

      arrayIndex = "Auto",      ---@type "Enable"|"Disable"|"Auto"
      paramName  = "All",       ---@type "All"|"Literal"|"Disable" Show hints of parameter name at the function call.
      semicolon  = "SameLine",  ---@type "All"|"SameLine"|"Disable" Whether to show a hint to add a semicolon to the end of a statement.
    ...
    type = {
      ...
      weakNilCheck   = true,  -- Whether it is permitted to assign a union type that contains nil to a variable that does not permit it.
